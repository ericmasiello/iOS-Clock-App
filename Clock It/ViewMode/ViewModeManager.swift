...
class ViewModeManager {
  static func mode(by current: Date, wakeup initialWakeup: Date, wakeupDuration: Double) -> ViewMode {
    ...
    let currentDateComponents = Calendar.current.dateComponents([.day, .month, .year], from: current)
    ...
    guard let wakeupStart = Calendar.current.date(bySettingHour: wakeupHour, minute: wakeupMinutes, second: 0, of: wakeupBase) else {
      return .dim
    }
